...
describe("Traversing DOM elements in Cypress", () => {
  ...
  it("children() to get the children of DOM elements", () => {
    cy.get('.traversal-breadcrumb').children('.active').should('contain', 'Contact Us') //uz pomoc children komande trazimo sve liste (l) unutar OL i onda klasom gadjamo listu koju zelimo 
  });
  ...
  it("eq() to retrieve a specific element based on index", () => {
    cy.get('.traversal-drinks-list > *').eq(2).should('contain', 'Milk') // sa klasom si upao u listu i sa zvezdicom si oznacio sve liste unutar iste
  });

  it("filter() to retrieve DOM elements that match a specific selector", () => {
    cy.get('.btn-group-toggle > *').filter('.active').should('contain', 'Button-1') // filter komanda radi isto sto i find()
  });
